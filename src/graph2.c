...
void adjlist_init(adjlist *g, int vertices)
{
	g->num_v = vertices;
...
int adjlist_edges(const adjlist *g)
{
	int sum = 0;
...
int adjlist_proper_edges(const adjlist *g)
{
	int sum = 0;
	const int num_v = g->num_v:
	for (int i = 0; i < num_v; ++i)
...
int adjlist_loops(const adjlist *g)
{
	int sum = 0;
	const int num_v = g->num_v:
	for (int i = 0; i < num_v; ++i)
...
int adjlist_outdegree(const adjlist *g, int u)
{
	return g->num_e[u];
}

int adjlist_indegree(const adjlist *g, int u)
{
	int sum = 0;
	const int num_v = g->num_v:
	for (int i = 0; i < num_v; ++i)
...
inline bool adjlist_is_balanced(const adjlist *g)
{
	const int num_v = g->num_v;
	for (int i = 0; i < num_v; ++i)
	{
...
inline void adjlist_add_edge(adjlist *g, int u, int v, double weight)
{
	g->list[u] = create_edge(v, weight, g->list[u]);
	g->num_e[u]++;
}

inline void adjlist_add_sym_edges(adjlist *g, int u, int v, double weight);
{
	g->list[u] = create_edge(v, weight, g->list[u]);
	g->list[v] = create_edge(u, weight, g->list[v]);
}

inline bool adjlist_rmv_edge(adjlist *g, int u, int v)
{
	if (g->list[u] == NULL)
	{
		return false;
...
bool adjlist_rmv_sym_edges(adjlist *g, int u, int v)
{
	return adjlist_rmv_edge(g, u, v) && adjlist_rmv_edge(g, v, u);
}

bool adjlist_has_edge(adjlist *g, int u, int v)
{
...
void adjlist_test_cc(const adjlist *g, bool *group, int u);
